...
puts walter.purchase(nanoblock) # Returns a new transaction
...
# FEATURES
# 1. Added methods for a customer to be able to return a product (if he has indeed bought that object)
...
# 2. Find_by method that returns an array of all the ocurrences of transactions involving a customer name, a product title or both

puts "By name: #{Transaction.find_by(customer: walter.name).count}" # Returns an array of 3 elements

julia = Customer.find_by_name("Julia Van Cleve")
julia.purchase(nanoblock)

puts "By title: #{Transaction.find_by(product: nanoblock.title).count}" # Returns an array of 4 elements

puts "By both: #{Transaction.find_by(customer: walter.name, product: nanoblock.title).count}" # Returns an array of 1 element
